    ...
    <div className="mx-auto max-w-3xl space-y-5 print:max-w-none">
      <div className="flex items-center justify-between no-print">
        <div className="flex items-center gap-2">
          <button onClick={() => navigate("/analytics")} className="btn-ghost">← {tr("ob_back")}</button>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <Card><SectionTitle>{tr("li_title")}</SectionTitle>
          {!moduleOn("listening") ? (
            <p className="text-sm text-ink-400">{tr("rp_modOff")}</p>
          ) : (
            ...
            <div className="space-y-1.5">
              {(objs || []).slice(0, 5).map((o, i) => (
                <div key={i} className="flex items-center justify-between text-sm">
                  ...
                  <span className="kpi-num ms-2 shrink-0 text-ink-800">{Math.round(safeNum(o.progress))}%</span>
                </div>
              ))}
            </div>
          )}</Card>

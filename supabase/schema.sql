-- Pulse (نبض) — The Marketing Operating Platform · Supabase / PostgreSQL schema
...
create table if not exists settings (
  ...
  "orgNameAr"   text not null default 'مؤسستك',
  ...
  "localCurrency"  text not null default 'SDG',
  "localCurrencyAr" text not null default 'ج.س',
  "businessUnits"  jsonb not null default '[]',
...
-- Targets that the rest of Pulse rolls up to. Progress is computed live
...
create table if not exists lead_activities (
  ...
  "actorId"   uuid,
  "actorName" text,
  kind        text not null check (kind in ('CREATED','STAGE','NOTE','CAPTURE','TASK','WA')),
  body        text,
  meta        jsonb,
  "createdAt" timestamptz not null default now()
);
...
-- Wave 0 · Pulse foundation columns (parity with APPLY-LATEST for pre-existing databases)
...
-- and DELETE raise, even for the application role. createdAt sits
-- outside the hash (DB-assigned); its integrity rests on the trigger.
alter table audit_log add column if not exists seq bigserial;
alter table audit_log add column if not exists "prevHash" text;
alter table audit_log add column if not exists "rowHash" text;

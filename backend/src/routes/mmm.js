...
mmmRouter.post("/fit", requirePerm("analytics"), async (req, res, next) => {
  try {
    ...
    const out = await fitModel({
      ...
      // W3·G2 — the seed makes every bootstrap interval reproducible; it
      // is stored with the run for auditability.
      seed: Number.isInteger(seedRaw) && seedRaw >= 0 ? seedRaw % 2147483647 : 42,
      userId: req.user.id,
    });
    if (!out.ok) return res.status(200).json(out);

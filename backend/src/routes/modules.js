...
// ── W4·UX2 · content through the approvals engine ────────────────────
// The composer's quick-draft path: one endpoint walks the item to
// REVIEW and files it with the SAME engine that governs slots and
// invoices — never a parallel mechanism. Approval stamps APPROVED (the
// publish gate opens); rejection returns it to IDEA.
export const contentExtraRouter = Router();
contentExtraRouter.use(requireAuth);
contentExtraRouter.post("/:id/request-approval", requirePerm("content"), async (req, res, next) => {
  try {
    const ci = await get(`SELECT * FROM content_items WHERE id = $1`, [req.params.id]);

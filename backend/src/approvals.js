...
approvalsRouter.post("/:id/decide", async (req, res, next) => {
  try {
    ...
    const ap = await get(`SELECT * FROM approvals WHERE id = $1`, [req.params.id]);
    if (!ap) return res.status(404).json({ error: "Not found" });
    if (ap.status !== "PENDING") return res.status(409).json({ error: "Already decided" });

    const rule = ENTITY_RULES[ap.entity];
    const allowed = req.user.permissions?.admin || (rule && hasPerm(req.user.permissions, rule.module, "write"));
    if (!allowed) return res.status(403).json({ error: "Insufficient permissions" });

    const note = req.body?.note ? String(req.body.note).slice(0, 2000) : null;
    await run(
      `UPDATE approvals SET status = $2, "approverId" = $3, note = COALESCE($4, note), "decidedAt" = now() WHERE id = $1`,
    ...
    res.json(await get(`SELECT * FROM approvals WHERE id = $1`, [ap.id]));
  } catch (e) { next(e); }
});
